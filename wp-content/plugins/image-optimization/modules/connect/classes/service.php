...
class Service {
	...
	public static function request( string $url, array $args, int $valid_response_code = 200 ): ?array {
		...
		if ( is_wp_error( $response ) ) {
			error_log( '[Connect Service]: ' . $response->get_error_message() );
			throw new Service_Exception( $response->get_error_message() );
		}

		if ( $valid_response_code !== wp_remote_retrieve_response_code( $response ) ) {
			error_log( '[Connect Service]: invalid status code' . wp_remote_retrieve_response_code( $response ) );
			throw new Service_Exception( wp_remote_retrieve_body( $response ) );
		}
