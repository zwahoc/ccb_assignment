...
class SettingsListener
{
     ...
     * @return void
     */
    private function onboarding_redirect(bool $success = \true): void
    {
    ...
    public function listen(): void
    {
        ...
        $credentials_change_status = null;
        // Cannot detect on Card Processing page.
        if (\WooCommerce\PayPalCommerce\WcGateway\Settings\Settings::CONNECTION_TAB_ID === $this->page_id) {
            $credentials_change_status = $this->determine_credentials_change_status($settings);
        }
        if (PayPalGateway::ID === $this->page_id) {
            $settings['enabled'] = isset($_POST['woocommerce_ppcp-gateway_enabled']) && 1 === absint($_POST['woocommerce_ppcp-gateway_enabled']);
        }
        ...
        if ($credentials_change_status) {
            ...
            if (in_array($credentials_change_status, array(self::CREDENTIALS_REMOVED, self::CREDENTIALS_CHANGED), \true)) {
    ...
    private function get_onboarding_redirect_url(): string
    {
         ...
         */
        return apply_filters('woocommerce_paypal_payments_onboarding_redirect_url', admin_url('admin.php?page=wc-settings&tab=checkout&section=ppcp-gateway&ppcp-tab=' . \WooCommerce\PayPalCommerce\WcGateway\Settings\Settings::CONNECTION_TAB_ID));
    }
    /**
    ...
    private function determine_credentials_change_status(array $new_settings): string
    {
        ...
        if ($had_credentials) {
            ...
            if ($current_id !== $new_id || $current_secret !== $new_secret) {
    ...
    private function retrieve_settings_from_raw_data(array $raw_data): array
    {
        ...
        foreach ($this->setting_fields as $key => $config) {
            if (!in_array($this->state->current_state(), $config['screens'], \true)) {
                continue;
            }
            ...
            switch ($config['type']) {
                ...
                case 'ppcp-multiselect':
                    ...
                    foreach ($values as $index => $raw_value) {
                        ...
                        if (!in_array($value, array_keys($config['options']), \true)) {
                            continue;
                        }
                ...
                case 'select':
                    ...
                    $settings[$key] = isset($raw_data[$key]) && in_array(sanitize_text_field($raw_data[$key]), $options, \true) ? sanitize_text_field($raw_data[$key]) : null;
                    break;
            }
        }
        // phpcs:enable WordPress.Security.NonceVerification.Recommended
     ...
     */
    public function filter_settings(bool $condition, string $setting_slug, callable $filter_function, bool $persist = \true): void
    {
        if (!$this->is_valid_site_request() || State::STATE_ONBOARDED !== $this->state->current_state()) {
            return;
        }
