...
class SmartButton implements \WooCommerce\PayPalCommerce\Button\Assets\SmartButtonInterface
{
    ...
    {
        add_action($this->checkout_dcc_button_renderer_hook(), array($this, 'dcc_renderer'), 11);
        ...
        add_filter('woocommerce_credit_card_form_fields', function (array $default_fields, $id) use ($subscription_helper): array {
            if (CreditCardGateway::ID === $id && is_user_logged_in() && $this->settings->has('vault_enabled_dcc') && $this->settings->get('vault_enabled_dcc') && apply_filters('woocommerce_paypal_payments_should_render_card_custom_fields', \true)) {
                ...
                if ($tokens && $this->payment_token_repository->tokens_contains_card($tokens)) {
                    $output = sprintf('<p class="form-row form-row-wide"><label>%1$s</label><select id="saved-credit-card" name="saved_credit_card"><option value="">%2$s</option>', esc_html__('Or select a saved Credit Card payment', 'woocommerce-paypal-payments'), esc_html__('Choose a saved payment', 'woocommerce-paypal-payments'));
                    foreach ($tokens as $token) {
                        if (isset($token->source()->card)) {
                            $output .= sprintf('<option value="%1$s">%2$s ...%3$s</option>', $token->id(), $token->source()->card->brand, $token->source()->card->last_digits);
                        }
                    }
                    $output .= '</select></p>';
    ...
    private function render_message_wrapper_registrar(): bool
    {
        ...
        $get_hook = function (string $location) use ($default_pay_order_hook, $is_block_theme, $has_paylater_block): ?array {
            switch ($location) {
                ...
                case 'shop':
                    return $is_block_theme ? $this->messages_renderer_block($location, 'core/query-title', 10) : $this->messages_renderer_hook($location, 'woocommerce_archive_description', 10);
                case 'home':
                    return $is_block_theme ? $this->messages_renderer_block($location, 'core/navigation', 10) : $this->messages_renderer_hook($location, 'loop_start', 20);
                default:
                    return $has_paylater_block ? $this->messages_renderer_hook($location, 'ppcp_paylater_message_block', 10) : null;
            }
        };
        ...
        if ($hook['blockName'] ?? \false) {
            ...
            add_action($hook['name'], array($this, 'message_renderer'), $hook['priority']);
        }
        ...
        if ($location === 'pay-now' && $hook['name'] === $default_pay_order_hook && apply_filters('woocommerce_paypal_payments_put_pay_order_messages_before_payment_methods', \true)) {
            add_action('ppcp_after_pay_order_message_wrapper', function () {
                echo '
<script>
document.querySelector("#payment").before(document.querySelector(".ppcp-messages"))
    ...
    private function render_button_wrapper_registrar(): bool
    {
        ...
        if ($this->settings_status->is_smart_button_enabled_for_location('mini-cart') && !$this->is_free_trial_cart()) {
            add_action($this->mini_cart_button_renderer_hook(), function () {
        ...
        if (isset($available_gateways['ppcp-gateway'])) {
            add_action($this->pay_order_renderer_hook(), function (): void {
            ...
            add_action($this->checkout_button_renderer_hook(), function (): void {
            ...
            add_action($this->proceed_to_checkout_button_renderer_hook(), function () use ($enabled_on_cart) {
    ...
    {
        return $this->dcc_configuration->is_acdc_enabled() && in_array($this->context(), apply_filters('woocommerce_paypal_payments_can_render_dcc_contexts', array('checkout', 'pay-now', 'add-payment-method')), \true);
    }
    /**
    ...
    public function enqueue(): void
    {
        if ($this->can_render_dcc()) {
            wp_enqueue_style('ppcp-hosted-fields', untrailingslashit($this->module_url) . '/assets/css/hosted-fields.css', array(), $this->version);
        }
        wp_enqueue_style('gateway', untrailingslashit($this->module_url) . '/assets/css/gateway.css', array(), $this->version);
        wp_enqueue_script('ppcp-smart-button', untrailingslashit($this->module_url) . '/assets/js/button.js', array('jquery'), $this->version, \true);
        wp_localize_script('ppcp-smart-button', 'PayPalCommerceGateway', $this->script_data());
    }
    /**
    ...
    public function message_renderer($block_params = array()): void
    {
        ...
        if (is_array($block_params) && ($block_params['blockName'] ?? \false)) {
            $this->render_after_block($block_params['blockName'], '<div class="wp-block-group alignwide">' . $messages_placeholder . '</div>', $block_params['priority'] ?? 10);
        } else {
    ...
    private function render_after_block(string $name, string $content, int $priority = 10): void
    {
        add_filter(
            ...
            function ($block_content, $block_params) use ($name, $content, $priority) {
                if (is_array($block_params) && ($block_params['blockName'] ?? null) === $name) {
    ...
    public function dcc_renderer()
    {
        ...
        printf('<div id="ppcp-hosted-fields" style="display:none;">
                    ...
                    <style id="ppcp-hide-dcc">.payment_method_ppcp-credit-card-gateway {display:none;}</style>', esc_html($label));
    }
    /**
    ...
    private function has_subscriptions(): bool
    {
        ...
        if ($this->subscription_helper->accept_manual_renewals() && $this->paypal_subscriptions_enabled() !== \true) {
            return \false;
        }
        if (is_product()) {
    ...
    public function script_data(): array
    {
        ...
        if ($this->is_paypal_continuation()) {
            ...
            assert($order !== null);
            $localize['continuation'] = array('order_id' => $order->id());
        }
    ...
    {
        $attributes = array('data-partner-attribution-id' => $this->bn_code_for_context($this->context()));
    ...
    private function bn_codes(): array
    {
        ...
        return array('checkout' => $bn_code, 'cart' => $bn_code, 'mini-cart' => $bn_code, 'product' => $bn_code);
    }
    /**
    ...
    private function components(): array
    {
         ...
         */
        return array_unique((array) apply_filters('woocommerce_paypal_payments_sdk_components_hook', $components, $this->context()));
    }
    /**
    ...
    {
        return $this->get_style_value("{$apm}_button_{$style}") ?? ($default ? $this->normalize_style_value($default) : null) ?? $this->style_for_context($style, 'checkout');
    }
    /**
    ...
    private function messages_renderer_hook(string $location, string $default_hook, int $default_priority): array
    {
         ...
         */
        $hook = (string) apply_filters("woocommerce_paypal_payments_{$location_hook}_messages_renderer_hook", $default_hook);
        /**
         * The filter returning the action priority that will be used for rendering Pay Later messages.
         */
        $priority = (int) apply_filters("woocommerce_paypal_payments_{$location_hook}_messages_renderer_priority", $default_priority);
        return array('name' => $hook, 'priority' => $priority);
    }
    /**
    ...
    private function messages_renderer_block(string $location, string $default_block, int $default_priority): array
    {
         ...
         * The filter returning the action name that will be used for rendering Pay Later messages.
         */
        $block_name = (string) apply_filters("woocommerce_paypal_payments_{$location_hook}_messages_renderer_block", $default_block);
        /**
         * The filter returning the action priority that will be used for rendering Pay Later messages.
         */
        $priority = (int) apply_filters("woocommerce_paypal_payments_{$location_hook}_messages_renderer_block_priority", $default_priority);
        return array('blockName' => $block_name, 'priority' => $priority);
    }
    /**
    ...
    private function proceed_to_checkout_button_renderer_hook(): string
    {
         ...
         */
        return (string) apply_filters('woocommerce_paypal_payments_proceed_to_checkout_button_renderer_hook', 'woocommerce_proceed_to_checkout');
    }
    /**
    ...
    private function mini_cart_button_renderer_hook(): string
    {
         ...
         */
        return (string) apply_filters('woocommerce_paypal_payments_mini_cart_button_renderer_hook', 'woocommerce_widget_shopping_cart_after_buttons');
    }
    /**
    ...
    protected function product_supports_payment(WC_Product $product): bool
    {
         ...
         */
        return apply_filters('woocommerce_paypal_payments_product_supports_payment_request_button', !$product->is_type(array('external', 'grouped')) && $in_stock, $product);
    }
    /**
    ...
    public function is_button_disabled(string $context = null, array $context_data = array()): bool
    {
        ...
        if ('product' === $context) {
             ...
             */
            return apply_filters('woocommerce_paypal_payments_product_buttons_disabled', \false, $this->product_filter_context_data($context_data));
        }
        /**
         * Allows to decide if the button should be disabled globally or on a given context.
         */
        return apply_filters('woocommerce_paypal_payments_buttons_disabled', \false, $context);
    }
    /**
    ...
    private function is_pay_later_filter_enabled_for_location(string $location, array $context_data = array()): bool
    {
        if ('product' === $location) {
             ...
             * Allows to decide if the button should be disabled for a given product.
             */
            return !apply_filters('woocommerce_paypal_payments_product_buttons_paylater_disabled', \false, $this->product_filter_context_data($context_data));
        }
        /**
    ...
    {
        return $this->is_pay_later_filter_enabled_for_location($location, $context_data) && $this->settings_status->is_pay_later_button_enabled_for_location($location);
    }
    /**
    ...
    {
        return $this->is_pay_later_filter_enabled_for_location($location, $context_data) && $this->settings_status->is_pay_later_messaging_enabled_for_location($location);
    }
    /**
        ...
        add_filter(
            ...
            function ($value) {
                if (null === $value) {
                    ...
                    return in_array($this->context(), array('cart', 'checkout', 'cart-block', 'checkout-block'), \true);
                }
                return in_array($this->context(), array('cart-block', 'checkout-block'), \true) ? \true : $value;
            },
            11
        );
    }
