...
function blocksy_get_avatar_url($args = []) {
	$args = wp_parse_args($args, [
		'size' => 96,

		// User ID | WP_Comment | WP_Post
		'avatar_entity' => '__default__'
	]);

	if ($args['avatar_entity'] === '__default__') {
	...
	function blocksy_author_social_channels($args = []) {
		...
		$descriptor = [
			...
			'twitter' => [
				...
				'icon' => '<svg class="ct-icon" width="12" height="12" viewBox="0 0 20 20"><path d="M2.9 0C1.3 0 0 1.3 0 2.9v14.3C0 18.7 1.3 20 2.9 20h14.3c1.6 0 2.9-1.3 2.9-2.9V2.9C20 1.3 18.7 0 17.1 0H2.9zm13.2 3.8L11.5 9l5.5 7.2h-4.3l-3.3-4.4-3.8 4.4H3.4l5-5.7-5.3-6.7h4.4l3 4 3.5-4h2.1zM14.4 15 6.8 5H5.6l7.7 10h1.1z"/></svg>'
			],
			'instagram' => [
		...
		foreach ($additional_descriptors as $field) {
			if (
				isset($field['id'])
				&&
				isset($field['name'])
...
function blocksy_author_box() {
	...
	$type = blocksy_get_theme_mod($prefix . '_single_author_box_type', 'type-2');

	$wrapper_tag = blocksy_get_theme_mod($prefix . '_single_author_box_name_heading', 'h5');

	$has_author_box_social = blocksy_get_theme_mod(
		$prefix . '_single_author_box_social',
		'yes'
	) === 'yes';

	$has_author_box_posts_count = blocksy_get_theme_mod(
	...
	$deep_link_args = [
		'prefix' => $prefix,
		'suffix' => $prefix . '_has_author_box',
		'shortcut' => 'border'
	];

	$with_link = blocksy_get_theme_mod($prefix . '_single_author_box_archive_link', 'yes') === 'yes';
...
if (! function_exists('blocksy_get_featured_image_output')) {
	function blocksy_get_featured_image_output() {
		...
		$has_video_thumbnail = blocksy_akg_or_customizer(
			'video_thumbnail',
			$featured_image_source,
			'no'
		);
		...
		return apply_filters(
			'post_thumbnail_html',
			blocksy_html_tag('figure', ['class' => $class], blocksy_media([
